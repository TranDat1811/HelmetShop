<?php 
    ...
    $result_hanghoa = mysql_query($sql_hanghoa);
    
    $sql_tennhom="SELECT * FROM nhomhanghoa WHERE MaNhom='$_GET[MaNhom]' ";
    $result_tennhom = mysql_query($sql_tennhom);
    $row_tennhom=mysql_fetch_array($result_tennhom);
?>                   
<div class="product">
<!-- Chi tiết nhóm hàng hóa -->
<p class="list-header"><?php echo $row_tennhom['TenNhom'] ?></p>
        <?php 
                        while ($row_hanghoa=mysql_fetch_array($result_hanghoa)) {
                    ?>
